...
//    }
    ...
    public void transfer(String fromAccountNum, String toAccountNum, Double amount)
            throws InterruptedException, BankException {
        final Supplier<Double> allMoneyBeforeTrans = ()-> {
            Double s = 0D;
            for (Account sumAllMoney: accounts.values()) {
                s += sumAllMoney.getMoney();
            }
        ...
        if (allMoneyBeforeTrans != allMoney)
            throw new BankException("Money before transaction doesn't equal after");
